...
def plots_kmeans(dataset, k_min=2, k_max=10, range_type="linear", nb_values=10, method='elbow', inp_algo='auto',
                 inp_init='k-means++', inp_n_init=10):
    """
    ...
    if range_type == "log":
        k_values = list(np.logspace(k_min, k_max, num=nb_values).astype(int))
...
def kmeans(df, k, inp_algo='lloyd', inp_init='k-means++', inp_n_init=10):
    model = KMeans(n_clusters=k, algorithm=inp_algo, init=inp_init, n_init=inp_n_init)
    model.fit(df)
    return model
